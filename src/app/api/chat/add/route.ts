...
export async function POST(req: Request) {
  try {
    ...
    if (idToAdd === session.user.id) {
      return new Response("Вы не можете добавить чат с самим собой", {status: 400}) 
    }

    const isAlredyAddedFirstToSecond = await db.sismember(`user:${idToAdd}:incoming_chat_requests`, session.user.id);

    if (isAlredyAddedFirstToSecond) {
      return new Response("Чат уже существует", {status: 400})
    }

    const isAlredyAddedSecondToFirst = await db.sismember(`user:${session.user.id}:chats`, idToAdd);
